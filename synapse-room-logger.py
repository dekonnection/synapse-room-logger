...
class SynapseRoomLogger(object):
    def __init__(self, config):
        ...
        self.state_file_path = '{}/{}'.format(config['output_directory'], state_file_name)
    ...
    def db_connect(self):
        ...
        try:
            ...
            logging.error("Could not connect to database : \"{}\"".format(str(e).rstrip()))
            return False
    ...
    def request_messages(self):
        ...
        self.cur.execute(base_query.format(room_id=self.room_id, after_ts=self.last_ts_written))
        ...
        for row in self.cur:
            ...
            if self.append_line(file_path, json.dumps(line)):
                logging.info("Message with timestamp {} written to {}".format(line['ts'], file_path))
                self.last_ts_written = line['ts']
            else:
                logging.error("We couldn't write message {}, we will exit.".format(line['ts']))
                return False
    ...
    def append_line(self, file_path, line):
        try:
            ...
            logging.error("Could not write {}, maybe the parent directory is missing.".format(file_path))
            return False
        except PermissionError:
            logging.error("Could not write {}, you don't have sufficient permissions to write here.".format(file_path))
            return False
    ...
    def read_last_ts_written(self):
        try:
            ...
            with open(self.state_file_path, "r") as file:
                ...
                logging.info("Last timestamp from previous run is {}.".format(self.last_ts_written))
                return True
        except FileNotFoundError:
            self.last_ts_written = 0
            logging.warn("No state file found at {}, setting last timestamp written to 0.".format(self.state_file_path))
            return False
    ...
    def write_last_ts_written(self):
        try:
            ...
            with open(self.state_file_path, "w") as file:
                ...
                logging.info("Timestamp {} written to state file.".format(self.last_ts_written))
                return True
        except FileNotFoundError:
            logging.error("Could not write {}, maybe the parent directory is missing.".format(file_path))
            return False
